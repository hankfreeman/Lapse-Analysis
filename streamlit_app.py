...
def map_nsf_reason(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for pattern, reason in REASON_MAP.items():
        mask = df["Clean_Reason"].astype(str).str.contains(
...
def load_and_merge_data(
    ...
    if "plan_code" in df_policies.columns:
        df_policies["plan_code"] = (
            df_policies["plan_code"].astype(str).str.strip().str.upper()
        )
...
def assign_termination_reason(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for pattern, reason in REASON_MAP.items():
        mask = df["Termination_Reason"].astype(str).str.contains(
            pattern, case=False, na=False
        )
...
def calculate_initial_mixes(df_cohort: pd.DataFrame) -> pd.DataFrame:
    ...
    for segment_col in ["underwriting_class", "gender", "tobacco"]:
        if segment_col in df_cohort.columns:
            ...
            counts = (
                df_cohort.groupby(["Cohort_Week_Index", segment_col])["policy_nbr"]
                .nunique()
            ...
            totals = (
                ...
                .nunique()
                .reset_index(name="Total_Cohort_Policies")
            )
            ...
            counts["Proportion"] = (
                counts["Policy_Count"] / counts["Total_Cohort_Policies"]
            ) * 100
...
def calculate_average_months_held(
    df: pd.DataFrame,
    analysis_end_date: pd.Timestamp,
    cohort_dates_df: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    for cohort_index in df_calc["Cohort_Week_Index"].unique():
        ...
        for col in available_cols:
            if col != "Cohort_Week_Index":
                segment_avg = (
                    df_cohort.groupby(col)["Months_Held"]
                    .agg(["mean", "count"])
                    .reset_index()
                )
...
def perform_cohort_analysis(
    ...
    df_cohort["Cohort_Week_Index"] = (
        (df_cohort["app_recvd_date"] - cohort_start_date).dt.days // 7 + 1
    )
    cohort_dates = df_cohort.groupby("Cohort_Week_Index")["app_recvd_date"].min().rename(
        "Cohort_Start_Date"
    )
    ...
    df_cohort["Actual_Lapse_Date"] = df_cohort["term_date"].where(
    ...
    if "issue_date" in df_cohort.columns and pd.api.types.is_datetime64_any_dtype(
        ...
        df_cohort["Policy_Month"] = (
            (
                (
                    df_cohort["Actual_Lapse_Date"] - df_cohort["issue_date"]
                ).dt.days
                / 30.4375
            )
            .fillna(0)
    ...
    df_cohort["Effective_End_Date"] = df_cohort["Actual_Lapse_Date"].fillna(
        ANALYSIS_END_DATE
    )
    df_cohort["Tenure_Weeks"] = (
        (df_cohort["Effective_End_Date"] - df_cohort["app_recvd_date"]).dt.days / 7
    )
    df_cohort["Lapse_Week"] = (
        (df_cohort["Actual_Lapse_Date"] - df_cohort["app_recvd_date"]).dt.days // 7 + 1
    )
    ...
    total_lapsed_policies = df_cohort[df_cohort["Lapsed"] == 1][
        "policy_nbr"
    ].nunique()

    lapse_counts = (
        df_cohort[df_cohort["Lapsed"] == 1]
        .groupby(["Cohort_Week_Index", "Lapse_Week"])["policy_nbr"]
        .nunique()
        .rename("Lapses")
    )
    lapse_matrix = (
        lapse_counts.reset_index()
        .pivot_table(
            index="Cohort_Week_Index",
            columns="Lapse_Week",
            values="Lapses",
            fill_value=0,
        )
...
def aggregate_average_months_held(
    ...
    df_filtered = df_avg[
        (df_avg["Cohort_Date"] >= start_date) & (df_avg["Cohort_Date"] <= end_date)
    ].copy()
    ...
    summary_df["Avg_Months_Held"] = (
        summary_df["Total_Weighted_Months"] / summary_df["Total_Policies"]
    )
...
def plot_segment_avg_months(
    df_avg: pd.DataFrame, segment: str, title: str, key: str
) -> None:
    """Generates a bar chart for average months held by segment (Used for Summary Tab)."""
    segment_df = df_avg[df_avg["Segment"] == segment].sort_values(
        "Avg_Months_Held", ascending=False
    )
...
def plot_segment_avg_months_single_cohort(
    df_avg: pd.DataFrame, cohort_index: int, segment: str, cohort_map: Dict[int, str]
) -> None:
    """Generates a bar chart for average months held for a single cohort, segmented by attribute."""

    df_cohort = df_avg[
        (df_avg["Cohort_Index"] == cohort_index) & (df_avg["Segment"] == segment)
    ...
    st.markdown(
        f"**Cohort Overall Average:** `{overall_avg:.2f} months` "
        f"(Policies: {df_cohort['Policies'].sum():,})"
    )
    ...
    st.plotly_chart(
        ...
        key=f"persistency_bar_{cohort_index}_{segment.replace(' ', '_')}",
    )


def plot_initial_mix_trends(
    df_mix: pd.DataFrame, segment: str, cohort_dates_df: pd.DataFrame
) -> None:
    ...
    fig = px.area(
        ...
        labels={
            "Cohort_Start_Date": "Cohort Week Start Date",
    ...
    st.plotly_chart(
        fig,
        use_container_width=True,
        key=f"mix_trend_{segment.replace(' ', '_')}",
    )
...
st.title("🚀 Local Cohort Retention and Termination Analysis")
st.markdown(
    f"Data is automatically loaded from the configured local directory: `{BASE_PATH}`"
)
...
try:
    ...
    with st.spinner("Scanning directory and merging files (This may take a moment)..."):
        initial_df, earliest_date = load_and_merge_data(
            POLICY_FILE_PATTERN,
            NSF_FILE_PATTERN,
            CANCELLED_FILE_PATTERN,
            cache_buster=csv_mtime,
        )

    if initial_df.empty or pd.isna(earliest_date):
        st.error(
            "❌ Could not load or merge data, or no valid 'app_recvd_date' found. "
            f"Please check the `BASE_PATH` ({BASE_PATH}) and ensure files are present."
        )
        st.stop()

    st.success(
        f"✅ Data loaded successfully. Total policies merged: {len(initial_df):,}"
    )
    ...
    cohort_start_date_input = st.sidebar.date_input(
        ...
        help=(
            "Select a cohort start date. Restricted to be no earlier than "
            f"{MIN_ALLOWED_DATE.strftime('%Y-%m-%d')}."
        ),
    )
    cohort_start_date = pd.to_datetime(cohort_start_date_input)

    analysis_end_date_input = st.sidebar.date_input(
        "Analysis Cut-off Date", value=ANALYSIS_END_DATE
    )
    ANALYSIS_END_DATE = pd.to_datetime(analysis_end_date_input)

    st.sidebar.header("2. Data Filtering")

    def get_unique_values(df: pd.DataFrame, col_name: str) -> List[str]:
        if col_name in df.columns:
            return (
                df[col_name]
                .astype(str)
    ...
    selected_plans_for_analysis: List[str] = []

    if plan_group_selection == "All":
        selected_plans_for_analysis = all_plan_options
        st.sidebar.info(
            f"Filtering for **{len(selected_plans_for_analysis)}** total unique plan codes."
        )
    elif plan_group_selection == "Luminary":
        selected_plans_for_analysis = luminary_plan_codes
        st.sidebar.info(
            f"Filtering for **{len(selected_plans_for_analysis)}** Luminary plan codes."
        )
    elif plan_group_selection == "Other":
        selected_plans_for_analysis = other_plan_codes
        st.sidebar.info(
            f"Filtering for **{len(other_plan_codes)}** 'Other' plan codes."
        )
        ...
        st.sidebar.info(
            f"Custom selection: **{len(selected_plans_for_analysis)}** plan codes selected."
        )
    ...
    if "tobacco" in initial_df.columns:
        ...
        selected_tobacco = st.sidebar.multiselect(
            "Filter by Tobacco Status",
            options=tobacco_options,
            default=tobacco_options,
        )
    ...
    selected_case_status = st.sidebar.multiselect(
        ...
        help=(
            "Active: Not lapsed as of Analysis Cut-off Date. "
            "Inactive: Lapsed as of Analysis Cut-off Date."
        ),
    )
    ...
    if retention_matrix.empty:
        st.warning(
            "No policies found matching the selected filters and cohort start date."
        )
        st.stop()
    ...
    cohort_map = {
        index: f"Cohort {index} (Week of {date.strftime('%Y-%m-%d')})"
    ...
    (
        ...
        persistency_deep_dive_tab,
        historical_mixes_tab,
    ) = st.tabs(
        [
            "📊 Cohort Deep Dive",
    ...
    with cohort_tab:
        ...
        st.dataframe(
            color_retention_matrix(retention_matrix),
            use_container_width=True,
            key="retention_matrix_df",
        )
        st.markdown("---")
        ...
        plot_df = (
            ...
            .melt(
                id_vars="Cohort_Week_Index",
                var_name="Tenure_Week",
                value_name="Retention_Rate",
            )
        )
        plot_df["Tenure_Week_Num"] = plot_df["Tenure_Week"].str.split("_").str[1].astype(
            int
        )
        ...
        fig = px.line(
            ...
            x="Tenure_Week_Num",
            y="Retention_Rate",
            color="Cohort_Week_Index",
            title="Retention Rate by Cohort",
            markers=True,
            color_discrete_sequence=px.colors.qualitative.Bold,
        )
        fig.update_layout(
            xaxis_title="Tenure Week",
        ...
        selected_cohort_index = list(cohort_map.keys())[0]
        ...
        group_col_name = {
            "Underwriting Class": "underwriting_class",
            "Gender": "gender",
            "Tobacco": "tobacco",
        }[group_by_col]
        ...
        max_tenure_cohort = (
            int(single_mix_df["Tenure_Weeks"].max()) if not single_mix_df.empty else 0
        )
        mix_data: List[pd.DataFrame] = []
        single_retention_df = plot_df[
            plot_df["Cohort_Week_Index"] == selected_cohort_index
        ].dropna(subset=["Retention_Rate"])

        for week_num in range(1, max_tenure_cohort + 1):
            policies_active_at_end_of_week = single_mix_df[
                (single_mix_df["Lapsed"] == 0)
                | (single_mix_df["Lapse_Week"] > week_num)
            ]
            if policies_active_at_end_of_week.empty:
                break

            if group_col_name in policies_active_at_end_of_week.columns:
                group_counts = (
                    policies_active_at_end_of_week.groupby(group_col_name)["Lapsed"]
                    .count()
                    .reset_index(name="Active_Policies")
                )
                total_active = group_counts["Active_Policies"].sum()
                if total_active > 0:
                    group_counts["Proportion_Active"] = (
                        group_counts["Active_Policies"] / total_active
                    ) * 100
        ...
        if mix_data and not single_retention_df.empty:
            ...
            for group in final_mix_df[group_col_name].unique():
                ...
                fig_combined.add_trace(
                    go.Scatter(
                        x=group_data["Tenure_Week_Num"],
                        y=group_data["Proportion_Active"],
                        name=f"Mix: {group}",
                        mode="lines",
                        fill="tonexty",
                        line=dict(width=0),
                        ...
                        hovertemplate=(
                            "Week: %{x}<br>"
                            f"{group_by_col}: {group}<br>"
                            "Mix: %{y:.1f}%<extra></extra>"
                        ),
                    ),
                    secondary_y=False,
                )
            fig_combined.add_trace(
                go.Scatter(
                    x=single_retention_df["Tenure_Week_Num"],
                    y=single_retention_df["Retention_Rate"],
                    name="Overall Retention Rate",
                    mode="lines+markers",
                    line=dict(color="black", width=3),
                    marker=dict(symbol="circle", size=6),
                    hovertemplate=(
                        "Week: %{x}<br>Retention: %{y:.2f}%<extra></extra>"
                    ),
                ),
                secondary_y=True,
            )
            fig_combined.update_layout(
                title_text=(
                    f"Retention Curve (Line) & Policy Mix (Area) for "
                    f"{cohort_map[selected_cohort_index]}"
                ),
                height=500,
                hovermode="x unified",
            ...
            fig_combined.update_yaxes(
                title_text="Active Policy Mix (%)",
                secondary_y=False,
                range=[0, 100],
                showgrid=False,
            )
            fig_combined.update_yaxes(
                title_text="Overall Retention Rate (%)",
                secondary_y=True,
                range=[0, 100],
                showgrid=True,
            )
            ...
            fig_bars = px.bar(
                ...
                barmode="group",
                title=f"Active Policy Counts for {cohort_map[selected_cohort_index]}",
            )
            fig_bars.update_layout(
                xaxis_title="Tenure Week",
                yaxis_title="Count of Active Policies",
                legend_title_text=group_by_col,
                height=400,
            )
            st.plotly_chart(
                fig_bars,
                use_container_width=True,
                key="active_policy_count_plot",
            )
            st.markdown("---")
            ...
            final_mix_df_display.rename(
                columns={
                    "Tenure_Week_Num": "Tenure Week",
                    group_col_name: "Category",
                    "Active_Policies": "Active Policy Count",
                    "Proportion_Active": "Mix (%)",
                },
                inplace=True,
            )
            st.dataframe(
                final_mix_df_display[
                    ["Tenure Week", "Category", "Active Policy Count", "Mix (%)"]
                ].style.format({"Mix (%)": "{:.2f}"}),
                use_container_width=True,
                key="ref_mix_data_deep_dive",
            )
    ...
    with persistency_summary_tab:
        ...
        st.info(
            "This view aggregates the Average Months Held for all cohorts that started "
            "within the selected time period, segmented by policy attributes."
        )

        if avg_months_held_df.empty:
            st.warning(
                "No data available to calculate average months held with current filters."
            )
        else:
            ...
            with col_date1:
                start_date_filter = st.date_input(
                    "Filter Cohort Start Date (From)",
                    value=min_date,
                    min_value=min_date,
                    max_value=max_date,
                    key="persistency_summary_start",
                )
            with col_date2:
                end_date_filter = st.date_input(
                    "Filter Cohort Start Date (To)",
                    value=max_date,
                    min_value=min_date,
                    max_value=max_date,
                    key="persistency_summary_end",
                )

            summary_data = aggregate_average_months_held(
                avg_months_held_df,
                pd.to_datetime(start_date_filter),
            ...
            if summary_data.empty:
                ...
                if (
                    not overall_summary_df.empty
                    and overall_summary_df["Policies"].sum() > 0
                ):
                    overall_avg_months = (
                        overall_summary_df["Avg_Months_Held"]
                        * overall_summary_df["Policies"]
                    ).sum() / overall_summary_df["Policies"].sum()
                    st.metric(
                        "Overall Average Months Held (Filtered Cohorts)",
                        f"{overall_avg_months:.2f} months",
                        delta=None,
                    )
                else:
                    st.metric(
                ...
                plot_segment_avg_months(
                    summary_data,
                    "Underwriting Class",
                    "Aggregated Avg Months Held by Underwriting Class",
                    "persistency_uw_summary",
                )
                ...
                plot_segment_avg_months(
                    summary_data,
                    "Gender",
                    "Aggregated Avg Months Held by Gender",
                    "persistency_gender_summary",
                )
                ...
                plot_segment_avg_months(
                    summary_data,
                    "Tobacco",
                    "Aggregated Avg Months Held by Tobacco Status",
                    "persistency_tobacco_summary",
                )
                ...
                st.dataframe(
                    summary_data.round({"Avg_Months_Held": 2}).rename(
                        columns={
                            "Policies": "Policy Count",
                            "Avg_Months_Held": "Avg Months Held",
                        }
                    ),
                    use_container_width=True,
                    key="persistency_aggregated_table",
                )

    # ----------------------------------------------------------------------------------
    # --- TAB 3: Single Cohort Persistency Deep Dive ---
    # ----------------------------------------------------------------------------------
    with persistency_deep_dive_tab:
        st.header("🔎 Single Cohort Persistency Deep Dive")
        st.info(
            "This view isolates one selected cohort and analyzes the average months "
            "held for its sub-segments."
        )
        ...
        if avg_months_held_df.empty:
            ...
            st.subheader(
                "Reference Data: Segmented Persistency Metrics (Selected Cohort)"
            )
            single_cohort_persistency_data = avg_months_held_df[
                (avg_months_held_df["Cohort_Index"] == selected_persistency_cohort_index)
                & (avg_months_held_df["Segment"] != "Cohort Overall")
            ].copy()
            single_cohort_persistency_data = single_cohort_persistency_data[
                ["Segment", "Category", "Avg_Months_Held", "Policies"]
            ].rename(
                columns={
                    "Policies": "Policy Count",
                    "Avg_Months_Held": "Avg Months Held",
            ...
            st.dataframe(
                single_cohort_persistency_data.style.format(
                    {"Avg Months Held": "{:.2f}"}
                ),
                use_container_width=True,
                key="ref_persistency_single_cohort_data",
            )
            ...
            cohort_summary_table["Cohort"] = (
                "Cohort "
                + cohort_summary_table["Cohort_Index"].astype(str)
                + " ("
                + cohort_summary_table["Cohort_Date"].dt.strftime("%Y-%m-%d")
                + ")"
            )

            final_table = cohort_summary_table[
                ["Cohort", "Avg_Months_Held", "Policies"]
            ].rename(
                columns={
                    "Avg_Months_Held": "Avg Months Held",
                    "Policies": "Policy Count",
            ...
            st.dataframe(
                final_table.style.format({"Avg Months Held": "{:.2f}"}),
                use_container_width=True,
                key="persistency_summary_table_all",
            )

    # ----------------------------------------------------------------------------------
    # --- TAB 4: Historical Mixes ---
    # ----------------------------------------------------------------------------------
    with historical_mixes_tab:
        st.header("📈 Initial Policy Mix Trends by Cohort")
        st.info(
            "This view shows how the demographic and underwriting mix of your new "
            "business has changed over time for the policies included in the analysis."
        )
        ...
        if initial_mix_df.empty:
            ...
            mix_table = pd.merge(
                initial_mix_df, cohort_dates_df.reset_index(), on="Cohort_Week_Index"
            )
            mix_table["Cohort Date"] = mix_table["Cohort_Start_Date"].dt.strftime(
                "%Y-%m-%d"
            )
            mix_table = mix_table[
                [
            ...
            mix_table.rename(
                columns={
                    "Cohort_Week_Index": "Cohort Index",
                    "Policy_Count": "Count",
            ...
            st.dataframe(
                mix_table.round({"Proportion (%)": 2}),
                use_container_width=True,
                hide_index=True,
                key="initial_mix_data_table",
            )

except Exception as e:
    st.error(f"A critical error occurred during file processing or analysis: {e}")
    st.info(
        "Double-check that the `BASE_PATH` is correct and all required files "
        "(`Policies_DBGA_*.csv`, `NSF_Activity_DBGA_*.csv`, `Cancelled_*.csv`) "
